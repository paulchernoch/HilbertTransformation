...
namespace Clustering
{
    ...
    public class HilbertIndex
    {
        ...
        public Compact Compress(Dictionary<int, UnsignedPoint> idsToPoints)
        {
            return new Compact(this, idsToPoints);
        }
        ...
        /// 
        /// If you create many Compact indices, the incremental memory requirement is proportional to N but not D.
        /// </summary>
        public class Compact
        {
            ...
            public Compact(HilbertIndex index, Dictionary<int, UnsignedPoint> idToPoints)
            {
            ...
            /// <param name="rangeLength">Number of points to draw from each index.</param>
            /// <returns>A value from zero to 1.0, where 1.0 means perfectly accurate.</returns>
            public static double CompositeAccuracy(IList<Compact> indices, UnsignedPoint point, int k, int rangeLength)
            {
                // Note the tricky use of Equivalent. The points from different indices should not be directly compared,
                // so we need to map a point from the first index to the equivalent point in another, then map back
                // for the final tally.
            ...
            public static double CompositeAccuracy(IList<Compact> indices, int k, int rangeLength, int sampleSize = 0)
            {
                var firstIndex = indices[0];
                if (sampleSize == 0)
                    sampleSize = firstIndex.Count;
